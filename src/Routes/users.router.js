...
const users = Router()
import { auth, lockUser, accessDeniedAdmin, accessDeniedforGithub } from "../middlewares/auth.js"
...
// RUTA "GET" QUE MUESTRA UNA TABLA CON EL NOMBRE/EMAIL/ROLE DE TODOS LOS USUARIOS DE LA BASE DE DATOS. (VISTA DISPONIBLE SOLO PARA "ADMIN")
users.get("/api/users", addLogger, lockUser, allUsers)

// RUTA "DELETE" QUE ELIMINAR A TODOS LOS USUARIOS QUE NO HAYAN ENTRADO INICIADO SESION O SE HAYAN DESLOGUADO EN LAS ULTIMAS 48 HORAS
...
// users.get("/api/users/documents", addLogger, auth, authToken, accessDeniedforGithub, uploadPhotos)
...
users.delete("/api/users/:email", addLogger, deleteApiUser)

// RUTA "POST" QUE ACTUALIZA EL ROL DE UN USUARIO. (VISTA DISPONIBLE SOLO PARA "ADMIN")
users.post("/api/users/:email", addLogger, updateUserByIdPost)
